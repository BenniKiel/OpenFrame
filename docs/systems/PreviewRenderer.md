...
| `card` | Panel mit **`surface`**, **`padding`**, **`radius`** und optional **`interaction`** (`none` \| `lift` \| `glow`) für Hover-Presets; Kinder (z. B. `heading` / `text` / `image`) — `card-block.tsx`. |
...
| `text` | Fließtext: `text`, optional `as` (p/span), `maxWidth` (px), **`align`** (`start` \| `center` \| `end`), optionale **`sizeScale`** (`sm` \| `base` \| `lg` \| `xl`); **`tone`**, **`leading`**, **`tracking`** (Phase 1b). Optional **`props.when.sm|md|lg`** für `align` + `sizeScale` (min-width, scoped CSS). |
| `heading` | Überschrift: `text`, `level` 1–6, `align`, optional `as` (h1–h6 oder `p`), optionale **`sizeScale`** (`sm` \| `base` \| `lg` \| `xl` \| `2xl` \| `3xl`); **`tone`**, **`leading`**, **`tracking`** — `heading-block.tsx`. Optional **`props.when.sm|md|lg`** für `align` + `sizeScale` (min-width, scoped CSS). |
...
- **`text/heading.props.when` (min-width):** Analoge Keys **`sm` \| `md` \| `lg`** für typografische Teil-Overrides (`align`, `sizeScale`) via scoped CSS (`typography-responsive.ts`).
