 ...
 */
export const pageNodeSchema = z.lazy(() => z.object({
    id: z.string().min(1),
    type: z.string().min(1),
...
function collectDuplicateNodeIds(root) {
...
function duplicateIdsError(duplicateIds) {
 ...
 */
export function parsePageDocumentFromJson(text) {
    let raw;
    try {
        raw = JSON.parse(text);
    }
